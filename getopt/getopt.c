...
static int z_getopt_internal(int argc,
	char **argv,
	const char *optstring,
	const struct zbx_option *longopts,
	int *longind,
	int long_only)
{
	                             ...
	                             ))
	{
		...
		while (*s && *s != '=' && * s != '#')
			s++;
#else
		/*s指向长选项的最后结束位置 也可以通过--aaa=bbb这样指定*/
		while (*s && *s != '=')
			s++;
#endif
				...
				else
				{
					if (zbx_opterr)
					{
						...
						fprintf(stderr, "option `--%s' doesn't allow an argument\n", pfound->name);
						  ...
						  /* +option or -option */
						  fprintf(stderr, "option `%c%s' doesn't allow an argument\n", argv[zbx_optind - 1][0], pfound->name);
					}
					nextchar += strlen(nextchar);
					return BAD_OPTION;
				}
...
#if OFF
				if (c < 040 || c >= 0177)
					fprintf(stderr,"unrecognized option, character code 0%o", c);
				else
...
int z_getopt(int argc, char **argv, const char *optstring)
{
	return z_getopt_internal(argc,
		argv,
		optstring,
...
int z_getopt_long(int argc,
	char **argv,
	const char *options,
	const struct zbx_option *long_options,
	int *opt_index)
{
	return z_getopt_internal(argc, argv, options, long_options, opt_index, 0);
}
